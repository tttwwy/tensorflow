...
def _extract_argmax_and_embed_encode(embedding, output_projection=None,
...
def _extract_argmax_and_embed(embedding, num_symbols, output_projection=None,
    ...
    def loop_function(prev, prev_probs, beam_size, i):
        if output_projection is not None:
            prev = nn_ops.xw_plus_b(prev, output_projection[0], output_projection[1])

        if i == 1:
          prev = array_ops.gather(prev,[0])
          prev_probs = [0]
...
def attention_decoder(decoder_inputs,decoder_inputs_encode_emb, initial_state, sub_theme_hidden_set, cell,
                      embedding_attention_seq2seq_scope,beam_size,
                      output_size=None, num_heads=1, loop_function=None,loop_function_encode=None,
                      dtype=dtypes.float32, scope=None,
  ...
  with variable_scope.variable_scope(scope or "attention_decoder"):
    ...
    attn_size = sub_theme_hidden_set[0].get_shape()[1].value

    attention_vec_size = attn_size

    state = initial_state

    def attention(query, attention_states, flg):
      ...
      for a in xrange(num_heads):
        with variable_scope.variable_scope("Attention_%d" % a):
          ...
          # hidden_features[a] = tf.Print(hidden_features[a],[hidden_features[a]],message='hidden_feature')
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      if loop_function is not None and prev is not None:
        # prev_gene_set.append(loop_function_encode(prev,i))
        # with variable_scope.variable_scope("loop_function", reuse=True):
        #   inp = loop_function(prev, i)
        #   prev_gene_set.append(inp)
        with variable_scope.variable_scope("loop_function", reuse=True):
          # do beam search
          inp, prev_probs, index, prev_symbol = loop_function(prev, prev_probs, beam_size, i)
          # index = tf.Print(index,[index],message='index',summarize=5)
          ...
          prev_gene_set.append(embedding_ops.embedding_lookup(loop_function_encode, prev_symbol))
          for j, output in enumerate(outputs):
              outputs[j] = array_ops.gather(output, index)   # update prev outputs
              # prev_gene_set[j] = array_ops.gather(prev_gene_set[j], index)
      ...
      # cal attention_state of every line

      if i ==0:
          with variable_scope.variable_scope('bidirectional_rnn', reuse=None):
              temp_encoder_outputs, temp_state_fw, temp_state_bw = rnn.bidirectional_rnn(cell, cell, decoder_inputs[:8],dtype=dtype)

      attn_flg = False

      def get_attention_states(encoder_outputs):

        top_states = [array_ops.reshape(e, [-1, 1, encoder_outputs[0].get_shape()[1].value]) for e in encoder_outputs]

        return array_ops.concat(1, top_states)

      if i%8 == 0:
        attn_flg = True

      ith_sub_poem = int(i/8)

      if ith_sub_poem == 0 and i%8 == 0:
          attention_states = get_attention_states([sub_theme_hidden_set[ith_sub_poem]])
      elif i%8 == 0 and i < len(decoder_inputs) - 2:
        with variable_scope.variable_scope(embedding_attention_seq2seq_scope,reuse=True):
          with variable_scope.variable_scope('bidirectional_rnn', reuse=True):

              history_info = [decoder_inputs_encode_emb[0]] + prev_gene_set + [decoder_inputs_encode_emb[-1]]

              # if ith_sub_poem == 1:
              #   for i_so,single_output in enumerate(history_info):
              #     history_info[i_so] = tf.Print(single_output,[single_output],message='single_output')

              encoder_outputs, state_fw, state_bw = rnn.bidirectional_rnn(cell, cell,history_info
                                  ,dtype=dtype)



              attention_states = get_attention_states([sub_theme_hidden_set[ith_sub_poem]] +encoder_outputs )
              # attention_states = tf.Print(attention_states,[attention_states],message='attention_state',summarize=10)

      # Run the attention mechanism.
      if i == 0 and initial_state_attention:
        with variable_scope.variable_scope(variable_scope.get_variable_scope(),
                                           reuse=True):
          attns = attention(state, attention_states, attn_flg)
      else:
        # state = tf.Print(state,[state],message='state',summarize=10)
        attns = attention(state, attention_states, attn_flg)
        # attns[0] = tf.Print(attns[0],[attns[0]],message='attns',summarize=10)
      ...
      if loop_function is not None:
        ...
        prev_gene_set.append(decoder_inputs_encode_emb[i+1])
      outputs.append(output)
...
def embedding_attention_decoder(decoder_inputs, initial_state, sub_theme_hidden_set,
                                cell,embedding_attention_seq2seq_scope, num_symbols, embedding_size, beam_size, num_heads=1,
                                output_size=None, output_projection=None,encode_embedding=None,
                                feed_previous=False,
                                update_embedding_for_previous=True,
                                dtype=dtypes.float32, scope=None,
                                initial_state_attention=False, cell_initializer=None):
  ...
  with variable_scope.variable_scope(scope or "embedding_attention_decoder"):
    embedding = variable_scope.get_variable("embedding",
                [num_symbols, embedding_size], initializer=cell_initializer,trainable=True)
    loop_function = _extract_argmax_and_embed(
                embedding, num_symbols, output_projection,
                update_embedding_for_previous) if feed_previous else None
    loop_function_encode = _extract_argmax_and_embed_encode(encode_embedding) if feed_previous else None

    batch_size = decoder_inputs[0].get_shape()[0].value
    emb_inp = [
        embedding_ops.embedding_lookup(embedding, i) for i in decoder_inputs]
    emb_inp_encode = [
        embedding_ops.embedding_lookup(encode_embedding, i) for i in  [[6807]*batch_size] + decoder_inputs[1:-1] + [[6792]*batch_size]]
    return attention_decoder(
        emb_inp,emb_inp_encode, initial_state, sub_theme_hidden_set, cell,embedding_attention_seq2seq_scope, beam_size, output_size=output_size,
        num_heads=num_heads, loop_function=loop_function,loop_function_encode=encode_embedding,
...
def embedding_attention_seq2seq(encoder_inputs, decoder_inputs, seq_len, cell,
                                ...
                                feed_previous=False, dtype=dtypes.float32,
                                scope=None, initial_state_attention=False, cell_initializer=None,sub_theme_num=4):
  ...
  Returns:
    A tuple of the form (outputs, state), where:
      ...
      state: The state of each decoder cell at the final time-step.
        It is a 2D Tensor of shape [batch_size x cell.state_size].
  """
  with variable_scope.variable_scope(scope or "embedding_attention_seq2seq",initializer=tf.random_uniform_initializer(minval=-0.08,maxval=0.08))\
          as embedding_attention_seq2seq_scope:
    # Encoder.
    # encoder_cell = rnn_cell.EmbeddingWrapper(
    #     cell, embedding_classes=num_encoder_symbols,
    #     embedding_size=embedding_size)
    ...
    sub_theme_hidden_set = []
    encoder_state = None

    embedding = variable_scope.get_variable("embedding",[num_encoder_symbols, embedding_size], initializer=cell_initializer,trainable=True)

    embed_encoder_inputs = [embedding_ops.embedding_lookup(embedding, i) for index, i in enumerate(encoder_inputs)]
    for ith_sub_theme in range(sub_theme_num):
        if ith_sub_theme == 0:
            reuse = None
        else:
            reuse = True

        with variable_scope.variable_scope('bidirectional_rnn',
                                           reuse=reuse):
            encoder_outputs, state_fw, state_bw = rnn.bidirectional_rnn(cell, cell, embed_encoder_inputs[ith_sub_theme*7:(ith_sub_theme+1)*7],
                                                                sequence_length=seq_len[ith_sub_theme], dtype=dtype)

            sub_theme_hidden_set.append(array_ops.concat(1,[state_fw,state_bw]))
    ...
    if isinstance(feed_previous, bool):
      return embedding_attention_decoder(
          decoder_inputs, encoder_state, sub_theme_hidden_set, cell,embedding_attention_seq2seq_scope,
          num_decoder_symbols, embedding_size, beam_size=beam_size, num_heads=num_heads,
          output_size=output_size, output_projection=output_projection,encode_embedding=embedding,
          feed_previous=feed_previous,
          initial_state_attention=initial_state_attention, cell_initializer=cell_initializer)
...
def model_with_buckets(encoder_inputs, decoder_inputs, targets, weights, seq_len,
                       ...
                       per_example_loss=False, name=None,sub_theme_num=4):
  ...
  with ops.op_scope(all_inputs, name, "model_with_buckets"):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
        ...
        bucket_outputs, _, bucket_symbols = seq2seq(encoder_inputs[:bucket[0]*sub_theme_num],
                                    decoder_inputs[:bucket[1]], seq_len)
