...
word2vec_model = gensim.models.Word2Vec.load_word2vec_format("planning_word2vec", binary=False)
...
def create_model(session, forward_only):
    ...
    model = seq2seq_model.Seq2SeqModel(
        ...
        FLAGS.learning_rate, FLAGS.learning_rate_decay_factor, FLAGS.beam_size,
        forward_only=forward_only,
        cell_initializer=tf.constant_initializer(np.array(word2vec_model.syn0, dtype=np.float32)))
...
def decode():
    with tf.Session() as sess:
        ...
        model.batch_size = 5  # We decode one sentence at a time.

        test_set = read_data('resource/train_resource/planning_train_poem_set.txt', '')
